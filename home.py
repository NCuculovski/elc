...
### END PASSWORD HERE
    ...
    def datamalt_demo():
        ...
        ***
        ...
        st.markdown(
            """
            A privacy-first, cost-efficient, data and tech-agnostic framework that enables integrated location-based media planning, activation, and measurement. Data.Malt is a fully integrated open-source ecosystem that links influential internal and external factors to the bottomline. It provides a single view of a geo performance across all available touchpoints, it enables prioritization of resources based on incremental value, and has forward-looking capabilities that anticipate consumption habits based on historical trend, seasonality, and recent local movements.  In its current Beta form, we would use Data.Malt to help plan local campaign executions and analyze geo-based performance.  However, in partnership with your data team, Data.Malt can be expanded to serve as a hub for custom planning and measurement tools - purpose built for the AlcBev industry.
